...
Everything starts with the `mini-git` command. It's built using Cobra, so it follows a similar structure to Git with subcommands. Right now we have `init` and `add`.
...
- **Branch References**: Basic branch reference system with HEAD tracking
